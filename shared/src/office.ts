...
export type OfficeIdentity = {
  ...
  /**
   * Explicit operator override for the short display (Building, District, City).
   * This is NOT a cache: when non-blank it intentionally wins over the
   * composed address from the structured fields. Leave blank to follow the
   * structured fields via `composeOfficeAddress`.
   */
  officeDisplayShort: string;
  /**
   * Explicit operator override for the full display (Line 1, Building,
   * District, City [postal], Region). Same override semantics as
   * `officeDisplayShort`: blank means "compose from structured fields".
   */
  officeDisplayFull: string;
};

/** Structured defaults; display strings are derived below via `composeOfficeAddress`. */
const DEFAULT_OFFICE_FIELDS = {
...
/**
 * Canonical default identity. Structured fields are the source of truth;
 * the display strings are generated by `composeOfficeAddress` so the default
 * can never drift from the structured address. Per-site configured display
 * strings remain supported as explicit overrides (see field docs).
 */
export const DEFAULT_OFFICE_IDENTITY: OfficeIdentity = {
  ...DEFAULT_OFFICE_FIELDS,
  officeDisplayShort: composeOfficeAddress(DEFAULT_OFFICE_FIELDS, 'short'),
  officeDisplayFull: composeOfficeAddress(DEFAULT_OFFICE_FIELDS, 'full'),
};

/**
 * Resolve the display string for a variant.
 * Fallback order:
 *   1. the configured display string for the variant (explicit operator
 *      override — blank means "compose from structured fields")
