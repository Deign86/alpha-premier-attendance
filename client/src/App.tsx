...
export function PayrollWorkspace({ users, profiles, records, onSaved }: { users: AdminUser[]; profiles: PayrollCalculationProfile[]; records: PayrollCutoffRecord[]; onSaved: () => void }) {
  ...
  const exportCsv = async () => { setExporting(true); try { const result = await exportPayrollCsv(); setMessage(result.success ? `Generated ${result.fileName}.` : result.error.message); } finally { setExporting(false); } };
