...
import type { ScanErrorResponse, ScanSuccessResponse, SetupUser, UserGender, AttendanceListItem, PayrollCalculationProfile, PayrollCutoffRecord, OfficeIdentity, LanStatusResponse, DatabaseInfoResponse } from '@rfid-attendance/shared';
...
type SetupForm = { userId: string; fullName: string; department: string; status: SetupUser['status']; employeeType: SetupUser['employeeType']; gender: UserGender | ''; dailyRate: string; photoUrl: string };
const emptySetupForm: SetupForm = { userId: '', fullName: '', department: '', status: 'ACTIVE', employeeType: 'INTERN', gender: '', dailyRate: '', photoUrl: '' };
...
export default function App() {
  ...
  const submit = useCallback(async (rawUid: string, source: 'RFID' | 'MANUAL_TEST') => {
    ...
    if (response.success) {
      if (response.action === 'TIME_IN') announceTimeIn(greetingForDate(new Date(), config.timezone), response.user.fullName, response.user.gender);
        ...
        {state === 'success' && success ? (
          <div className="kiosk-result is-success" role="status" aria-live="polite">
            ...
            <div className="result-meta">
              <span className={`employee-badge${success.user.employeeType === 'INTERN' ? ' intern-badge' : ''}`}>{success.user.employeeType}</span>
        ...
        {!props.token ? (
          ...
          <form className="setup-form" onSubmit={props.onUpsert}>
            ...
            <div className="setup-fields">
              <label><span className="field-label">User ID</span><input value={props.form.userId} onChange={(event) => props.onFormChange('userId', event.target.value)} autoComplete="off" required /></label>
              <label><span className="field-label">Full name</span><input value={props.form.fullName} onChange={(event) => props.onFormChange('fullName', event.target.value)} autoComplete="name" required /></label>
              <label><span className="field-label">Department / role <span className="optional">optional</span></span><input placeholder="IT / Admin" value={props.form.department} onChange={(event) => props.onFormChange('department', event.target.value)} autoComplete="organization" /></label>
              <label><span className="field-label">Status</span><select value={props.form.status} onChange={(event) => props.onFormChange('status', event.target.value)}><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></select></label>
              <label><span className="field-label">Employee type</span><select value={props.form.employeeType} onChange={(event) => props.onFormChange('employeeType', event.target.value)}><option value="INTERN">Intern</option><option value="EMPLOYEE">Employee</option></select></label>
              <label><span className="field-label">Gender</span><select value={props.form.gender} onChange={(event) => props.onFormChange('gender', event.target.value)}><option value="">Not set</option><option value="MALE">Male</option><option value="FEMALE">Female</option></select></label>
              {props.form.employeeType === 'EMPLOYEE' && <label><span className="field-label">Daily rate (PHP)</span><input type="number" min="0.01" step="0.01" required value={props.form.dailyRate} onChange={(event) => props.onFormChange('dailyRate', event.target.value)} /></label>}
...
type AdminUser = { userId: string; rfidUid: string; fullName: string; department: string | null; status: 'ACTIVE' | 'INACTIVE'; employeeType: 'INTERN' | 'EMPLOYEE'; gender: UserGender | null; dailyRate: number | null; payrollProfileId?: string | null; photoUrl?: string | null };
...
function UserEditor({ users, profiles, editing, setEditing, onSaved }: { users: AdminUser[]; profiles: PayrollCalculationProfile[]; editing: AdminUser | null; setEditing: (user: AdminUser | null) => void; onSaved: () => void }) {
  const blankUser: AdminUser = { userId: '', rfidUid: '', fullName: '', department: '', status: 'ACTIVE', employeeType: 'INTERN', gender: null, dailyRate: null, payrollProfileId: null };
