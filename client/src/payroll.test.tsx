...
function samplePayrollRecord(
  overrides: Partial<PayrollCutoffRecord> = {},
): PayrollCutoffRecord {
  return {
    payrollId: "P-001",
    employeeId: "EMP-001",
    ...
    grossCompensation: 5375,
    netPay: 5375,
    signaturePlaceholder: "________________",
    calculationBreakdown:
      "PHP 5,500.00 basic - PHP 250.00 half-day deduction = PHP 5,375.00",
    approvedWorkingDayOverage: false,
    status: "FINALIZED",
    finalizedAt: "2026-08-15T12:00:00+08:00",
    ...overrides,
  };
}
...
describe("PayrollWorkspace", () => {
  ...
  it("releases the save button and shows the backend error when cutoff creation fails", async () => {
    vi.mocked(savePayrollCutoff).mockRejectedValueOnce(
      "Employee and valid cutoff dates are required.",
    );
    ...
    await user.click(
      screen.getByRole("button", { name: "Save cutoff payroll" }),
    );

    expect(
      await screen.findByText("Employee and valid cutoff dates are required."),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Save cutoff payroll" }),
    ).toBeEnabled();
  ...
  it("renders a worksheet-style printable payroll layout for saved cutoffs via the employee print action", async () => {
    ...
    expect(screen.getAllByText("Grace Hopper").length).toBeGreaterThanOrEqual(
      2,
    );
    expect(screen.getAllByText("PHP 5,375.00").length).toBeGreaterThanOrEqual(
      6,
    );
  });
  ...
  it("renders a worksheet-style printable intern payroll sheet with the fixed PHP 80/day rule via the intern print action", async () => {
      ...
      <PayrollWorkspace
        ...
        records={[
          samplePayrollRecord({
            ...
            overtimeRate: 0,
            overtimePay: 0,
            grossCompensation: 770,
            netPay: 770,
            status: "DRAFT",
            calculationBreakdown:
              "PHP 800.00 basic - PHP 30.00 late deduction = PHP 770.00",
          }),
        ]}
        onSaved={vi.fn()}
      />,
    ...
    expect(
      screen.getAllByText("Total late hours").length,
    ...
    expect(screen.getAllByText("Total earnings").length).toBeGreaterThanOrEqual(
      1,
    );
  ...
  it("provides a fillable late-deduction form for employees and computes the deduction from hours × rate", async () => {
    vi.mocked(savePayrollCutoff).mockResolvedValueOnce({
      success: true,
    } as never);
    const user = userEvent.setup();
    render(
  ...
  it("renders the stored calculation-breakdown JSON as readable remarks instead of raw JSON", async () => {
        ...
        records={[
          samplePayrollRecord({
            calculationBreakdown: JSON.stringify({
              basicPayCentavos: 550000,
              totalCompensationCentavos: 550000,
              totalAllowanceCentavos: 0,
              lateDeductionCentavos: 0,
              halfDayDeductionCentavos: 25000,
              absenceDeductionCentavos: 0,
              overtimePayCentavos: 0,
              grossCompensationCentavos: 525000,
            }),
          }),
        ]}
        onSaved={vi.fn()}
    ...
    expect(
      screen.getByText(/Half-day deduction PHP 250\.00/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Gross compensation PHP 5,250\.00/),
    ).toBeInTheDocument();
  ...
  it("provides one print payroll action that renders each matching payroll template", async () => {
    ...
    expect(
      screen.queryByText("Intern Payroll Worksheet"),
