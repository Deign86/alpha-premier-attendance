# Voice Cloning Architecture & Maintainer Guide: "Ma'am Bea" (Voicebox AI Studio + Hybrid Splicing)
...
1. **Tier 1 — Existing Active Interns & Employees (100% Cloned Voice)**:
   Names of existing personnel are pre-rendered into individual audio clips (`names/<personId>.wav`) in Ma'am Bea's voice using Voicebox on the admin machine. When they scan, the kiosk plays:
...
> [!IMPORTANT]
> Voice generation is performed **exclusively on the admin/developer machine**. The kiosk PC **never runs Voicebox or GPU cloning inference**.

### Admin Hardware & Software Requirements
- **OS**: Windows 10/11 x64
- **GPU**: NVIDIA RTX 3050/4050 or higher (at least 4–6 GB VRAM recommended)
- **Voicebox AI Studio**: Installed and running locally ([`jamiepine/voicebox`](https://github.com/jamiepine/voicebox))
- **Voicebox API Endpoint**: `http://127.0.0.1:17493`
- **Voicebox Backend**: CUDA acceleration active (`POST /backend/download-cuda` or PyTorch `cu128`)
- **Active Voice Profile**: `"Ma'am Bea"` (Profile ID: `1ccbe006-2269-4c08-aa85-0167598232a1`, Engine: `Qwen-TTS 1.7B`)
...
### Step 1: Start Voicebox on the Admin PC
1. Launch Voicebox AI Studio (or ensure `voicebox.exe` is running).
2. Confirm the server is reachable by opening `http://127.0.0.1:17493/profiles` in a browser or running:
   ```bash
...
[Person ID: APG-2026-115] INTERN: "Ma. Teresa Carandang" -> Spoken: "Maria Teresa Carandang"
  [DRY-RUN] Would generate clip via Voicebox -> /voices/bea/names/APG-2026-115.wav
```
...
3. Sends generation requests to Voicebox `Qwen-TTS 1.7B` on CUDA.
...
### Step 4 (Alternative): Generate Directly from a Backup File (`.apbackup`)
If working offline from an exported application backup archive:

```bash
python scripts/generate_backup_names.py
```
...
4. **Delete Voicebox Profile**:
   Open Voicebox AI Studio, select the `"Ma'am Bea"` profile, and click **Delete Profile**.
...
| :--- | :--- | :--- |
| `Error connecting to Voicebox` | Voicebox app is closed | Launch Voicebox; verify port `17493` with `curl http://127.0.0.1:17493/profiles` |
| `Generation failed / CUDA error` | Missing CUDA backend | In Voicebox, trigger CUDA download or ensure NVIDIA GPU drivers are updated |
