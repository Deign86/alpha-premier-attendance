...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let db_path = args
        ...
        .find(|pair| pair[0] == "--db")
        .map(|pair| PathBuf::from(&pair[1]))
        .unwrap_or_else(|| PathBuf::from("attendance.db"));
...
fn inspect_csv(path: &PathBuf) -> Result<usize, Box<dyn std::error::Error>> {
    ...
    if headers.is_empty() {
        return Err(format!("{} has no header row", path.display()).into());
    }
    ...
    for field in required {
        if !headers.iter().any(|value| value == field) {
            return Err(format!("{} is missing required header {field}", path.display()).into());
        }
    }
...
fn number(row: &csv::StringRecord, index: usize) -> f64 {
    row.get(index)
        .unwrap_or("0")
        .trim()
        .parse::<f64>()
...
fn cents(row: &csv::StringRecord, index: usize) -> i64 {
    (number(row, index) * 100.0).round() as i64
}
...
async fn import_cutoffs(db: &SqlitePool, path: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for record in Reader::from_path(path)?.records() {
        ...
        sqlx::query(&query)
            ...
            .bind(cents(&row, 30))
            .bind(cents(&row, 31))
            .bind(row.get(32).filter(|v| !v.is_empty()))
            .bind(cents(&row, 33))
            .bind(cents(&row, 34))
            .bind(&row[35])
            .bind(&row[36])
            .bind(if row[37].eq_ignore_ascii_case("TRUE") {
                1
            } else {
                0
            })
            .bind(&row[38])
            .bind(row.get(39).filter(|v| !v.is_empty()))
            .bind(&now)
            .bind(&now)
            .execute(db)
            .await?;
...
async fn verify_counts(
    ...
    for (file, expected) in counts {
        let table = match *file {
            ...
            "AuditLogs.csv" => "audit_logs",
            "InternGrace.csv" => "intern_grace",
            "Payroll.csv" => "payroll",
            "PayrollProfiles.csv" => "payroll_profiles",
            "PayrollCutoffs.csv" => "payroll_cutoffs",
            _ => continue,
        };
        ...
        if actual < *expected as i64 {
            return Err(
                format!("{file}: imported {actual} rows but source contains {expected}").into(),
            );
        }
    }
    Ok(())
