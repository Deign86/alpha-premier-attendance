...
#[tauri::command]
async fn get_attendance(state: State<'_, AppState>, date: Option<String>) -> Result<serde_json::Value, String> {
    let selected = date.unwrap_or_else(|| chrono::Utc::now().with_timezone(&Manila).date_naive().to_string());
...
#[tauri::command]
async fn admin_users(state: State<'_, AppState>, token: String) -> Result<serde_json::Value, String> {
...
#[tauri::command]
async fn admin_list_users(state: State<'_, AppState>, token: String) -> Result<serde_json::Value, String> {
    admin_users(state, token).await
}

#[tauri::command]
async fn admin_upsert_user(state: State<'_, AppState>, token: String, user: serde_json::Value) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_AUTH_REQUIRED".into()); }
    let user_id = user.get("userId").and_then(|v| v.as_str()).unwrap_or("").trim();
    let rfid_uid = user.get("rfidUid").and_then(|v| v.as_str()).unwrap_or("").trim().to_ascii_uppercase();
    let full_name = user.get("fullName").and_then(|v| v.as_str()).unwrap_or("").trim();
    let status = user.get("status").and_then(|v| v.as_str()).unwrap_or("ACTIVE");
    let employee_type = user.get("employeeType").and_then(|v| v.as_str()).unwrap_or("INTERN");
    if user_id.is_empty() || rfid_uid.is_empty() || full_name.is_empty() || !matches!(status, "ACTIVE"|"INACTIVE") || !matches!(employee_type, "INTERN"|"EMPLOYEE") { return Err("ADMIN_VALIDATION_ERROR".into()); }
...
#[tauri::command]
async fn admin_create_user(state: State<'_, AppState>, token: String, user: serde_json::Value) -> Result<serde_json::Value, String> {
    admin_upsert_user(state, token, user).await
}

#[tauri::command]
async fn admin_update_user(state: State<'_, AppState>, token: String, user: serde_json::Value) -> Result<serde_json::Value, String> {
    admin_upsert_user(state, token, user).await
}

#[tauri::command]
async fn admin_delete_user(state: State<'_, AppState>, token: String, user_id: String) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_AUTH_REQUIRED".into()); }
    let result = sqlx::query("DELETE FROM users WHERE user_id = ?").bind(&user_id).execute(&state.db).await.map_err(|e| e.to_string())?;
    if result.rows_affected() != 1 { return Err("USER_NOT_FOUND".into()); }
    enqueue_sync(&state, "Users", &user_id, "DELETE", &serde_json::json!({"userId":user_id})).await;
    Ok(serde_json::json!({"success":true,"userId":user_id}))
}

#[tauri::command]
async fn admin_attendance(state: State<'_, AppState>, token: String, date: String) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_AUTH_REQUIRED".into()); }
    if !date.as_bytes().iter().enumerate().all(|(i, b)| if [4,7].contains(&i) { *b == b'-' } else { b.is_ascii_digit() }) || date.len() != 10 { return Err("INVALID_DATE".into()); }
    ...
    Ok(serde_json::json!({"success":true,"date":date,"attendance":attendance,"fetchedAt":chrono::Utc::now()}))
}

#[tauri::command]
async fn admin_list_attendance(state: State<'_, AppState>, token: String, date: String) -> Result<serde_json::Value, String> {
    admin_attendance(state, token, date).await
}

#[tauri::command]
async fn admin_update_attendance(state: State<'_, AppState>, token: String, attendance_id: String, payload: serde_json::Value) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_AUTH_REQUIRED".into()); }
    let row = sqlx::query("SELECT attendance_date,time_in,time_out,revision FROM attendance WHERE attendance_id=?").bind(&attendance_id).fetch_optional(&state.db).await.map_err(|e| e.to_string())?.ok_or_else(|| "ATTENDANCE_NOT_FOUND".to_string())?;
    let date = payload.get("attendanceDate").and_then(|v|v.as_str()).unwrap_or(row.get("attendance_date"));
    let time_in = payload.get("timeIn").and_then(|v|v.as_str());
    let time_out = payload.get("timeOut").and_then(|v|v.as_str());
    if chrono::NaiveDate::parse_from_str(date, "%Y-%m-%d").is_err() || time_in.is_some_and(|value| chrono::DateTime::parse_from_rfc3339(value).is_err()) || time_out.is_some_and(|value| chrono::DateTime::parse_from_rfc3339(value).is_err()) { return Err("ADMIN_VALIDATION_ERROR".into()); }
    let current_in: Option<String> = row.get("time_in");
    let current_out: Option<String> = row.get("time_out");
    let expected_in = payload.get("expectedTimeIn").and_then(|v|v.as_str()).or(current_in.as_deref());
    let expected_out = payload.get("expectedTimeOut").and_then(|v|v.as_str()).or(current_out.as_deref());
    let status = if time_in.is_some() && time_out.is_some() { "COMPLETED" } else if time_in.is_some() { "OPEN" } else { "INCOMPLETE" };
    let now = chrono::Utc::now().to_rfc3339();
    ...
    if updated.rows_affected() != 1 { return Err("ATTENDANCE_CONFLICT".into()); }
    let _ = sqlx::query("DELETE FROM payroll WHERE attendance_id=?").bind(&attendance_id).execute(&state.db).await;
    let _ = sqlx::query("DELETE FROM intern_grace WHERE attendance_id=?").bind(&attendance_id).execute(&state.db).await;
    ...
    Ok(serde_json::json!({"success":true,"attendanceId":attendance_id,"attendanceDate":date,"timeIn":time_in,"timeOut":time_out,"status":status}))
}

#[tauri::command]
async fn admin_delete_attendance(state: State<'_, AppState>, token: String, attendance_id: String, date: String) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_AUTH_REQUIRED".into()); }
    let result = sqlx::query("DELETE FROM attendance WHERE attendance_id=? AND attendance_date=?").bind(&attendance_id).bind(&date).execute(&state.db).await.map_err(|e|e.to_string())?;
    if result.rows_affected() != 1 { return Err("ATTENDANCE_NOT_FOUND".into()); }
    let _ = sqlx::query("DELETE FROM payroll WHERE attendance_id=?").bind(&attendance_id).execute(&state.db).await;
    let _ = sqlx::query("DELETE FROM intern_grace WHERE attendance_id=?").bind(&attendance_id).execute(&state.db).await;
    enqueue_sync(&state, "Attendance", &attendance_id, "DELETE", &serde_json::json!({"attendanceId":attendance_id,"attendanceDate":date})).await;
...
fn payroll_calculate_cutoff(input: serde_json::Value) -> Result<serde_json::Value, String> {
    ...
    let result = crate::services::cutoff_payroll::calculate(&crate::services::cutoff_payroll::CutoffInput { employee_id: input.get("employeeId").and_then(|v| v.as_str()).unwrap_or("").into(), employee_name: input.get("employeeName").and_then(|v| v.as_str()).unwrap_or("").into(), cutoff_start: input.get("cutoffStart").and_then(|v| v.as_str()).unwrap_or("").into(), cutoff_end: input.get("cutoffEnd").and_then(|v| v.as_str()).unwrap_or("").into(), daily_rate:value("dailyRate"), standard_working_days:value("standardWorkingDays"), actual_working_days:value("actualWorkingDays"), special_holiday_days:value("specialHolidayDays"), special_holiday_multiplier:value("specialHolidayMultiplier"), regular_holiday_days:value("regularHolidayDays"), regular_holiday_multiplier:value("regularHolidayMultiplier"), incentives_allowance:value("incentivesAllowance"), special_allowance:value("specialAllowance"), late_deduction:value("lateDeduction"), half_day_count:value("halfDayCount"), half_day_fraction:value("halfDayFraction"), absent_days:value("absentDays"), overtime_hours:value("overtimeHours"), overtime_rate:value("overtimeRate"), manual_adjustment:value("manualAdjustment"), adjustment_reason:input.get("adjustmentReason").and_then(|v| v.as_str()).map(str::to_owned), approved_working_day_overage:input.get("approvedWorkingDayOverage").and_then(|v| v.as_bool()).unwrap_or(false) })?;
...
#[tauri::command]
async fn payroll_list_profiles(state: State<'_, AppState>, token: String) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_AUTH_REQUIRED".into()); }
...
#[tauri::command]
async fn payroll_upsert_profile(state: State<'_, AppState>, token: String, profile: serde_json::Value) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_AUTH_REQUIRED".into()); }
    let id = profile.get("profileId").and_then(|v| v.as_str()).unwrap_or("").trim();
    let label = profile.get("label").and_then(|v| v.as_str()).unwrap_or("").trim();
    if id.is_empty() || label.is_empty() { return Err("ADMIN_VALIDATION_ERROR".into()); }
    let now = chrono::Utc::now().to_rfc3339();
    sqlx::query("INSERT INTO payroll_profiles (profile_id,label,payroll_frequency,standard_working_days_per_cutoff,incentives_allowance_centavos,special_allowance_centavos,special_holiday_multiplier,regular_holiday_multiplier,half_day_fraction,overtime_rate_centavos,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(profile_id) DO UPDATE SET label=excluded.label,standard_working_days_per_cutoff=excluded.standard_working_days_per_cutoff,incentives_allowance_centavos=excluded.incentives_allowance_centavos,special_allowance_centavos=excluded.special_allowance_centavos,special_holiday_multiplier=excluded.special_holiday_multiplier,regular_holiday_multiplier=excluded.regular_holiday_multiplier,half_day_fraction=excluded.half_day_fraction,overtime_rate_centavos=excluded.overtime_rate_centavos,revision=payroll_profiles.revision+1,updated_at=excluded.updated_at")
        .bind(id).bind(label).bind("SEMI_MONTHLY").bind(profile.get("standardWorkingDaysPerCutoff").and_then(|v| v.as_f64()).unwrap_or(11.0)).bind(profile.get("incentivesAllowance").and_then(|v| v.as_f64()).unwrap_or(0.0).mul_add(100.0, 0.0).round() as i64).bind(profile.get("specialAllowance").and_then(|v| v.as_f64()).unwrap_or(0.0).round() as i64 * 100).bind(profile.get("specialHolidayMultiplier").and_then(|v| v.as_f64()).unwrap_or(0.3)).bind(profile.get("regularHolidayMultiplier").and_then(|v| v.as_f64()).unwrap_or(1.0)).bind(profile.get("halfDayFraction").and_then(|v| v.as_f64()).unwrap_or(0.5)).bind(profile.get("overtimeRate").and_then(|v| v.as_f64()).unwrap_or(0.0).round() as i64 * 100).bind(&now).bind(&now).execute(&state.db).await.map_err(|e| e.to_string())?;
    enqueue_sync(&state, "PayrollProfiles", id, "UPSERT", &profile).await;
    Ok(serde_json::json!({"success":true,"profileId":id}))
}

#[tauri::command]
async fn payroll_list_cutoffs(state: State<'_, AppState>, token: String) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_AUTH_REQUIRED".into()); }
...
#[tauri::command]
async fn payroll_create_cutoff(state: State<'_, AppState>, token: String, input: serde_json::Value) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_AUTH_REQUIRED".into()); }
    let parsed = cutoff_input(&input);
    let result = crate::services::cutoff_payroll::calculate(&parsed)?;
    let id = uuid::Uuid::new_v4().to_string();
    let now = chrono::Utc::now().to_rfc3339();
    let profile = input.get("payrollProfileId").and_then(|v| v.as_str()).unwrap_or("BEA_STANDARD");
    let label = input.get("payrollCutoffLabel").and_then(|v| v.as_str()).unwrap_or("");
    ...
    sqlx::query(&insert_query)
        .bind(&id).bind(&parsed.employee_id).bind(&parsed.employee_name).bind(profile).bind(label).bind(&parsed.cutoff_start).bind(&parsed.cutoff_end).bind("SEMI_MONTHLY").bind((parsed.daily_rate*100.0).round() as i64).bind(parsed.standard_working_days).bind(parsed.actual_working_days).bind(result.basic_pay).bind(parsed.special_holiday_days).bind(parsed.special_holiday_multiplier).bind((result.total_compensation-result.basic_pay).max(0)).bind(parsed.regular_holiday_days).bind(parsed.regular_holiday_multiplier).bind(0_i64).bind((parsed.incentives_allowance*100.0).round() as i64).bind((parsed.special_allowance*100.0).round() as i64).bind(result.total_compensation).bind(result.total_allowance).bind(0.0_f64).bind(result.late_deduction).bind(parsed.half_day_count).bind(result.half_day_deduction).bind(parsed.absent_days).bind(result.absence_deduction).bind(parsed.overtime_hours).bind((parsed.overtime_rate*100.0).round() as i64).bind(result.overtime_pay).bind((parsed.manual_adjustment*100.0).round() as i64).bind(parsed.adjustment_reason.clone()).bind(result.gross_compensation).bind(result.net_pay).bind("").bind(breakdown.to_string()).bind(if parsed.approved_working_day_overage {1} else {0}).bind("DRAFT").bind(&now).bind(&now).execute(&state.db).await.map_err(|e| e.to_string())?;
...
#[tauri::command]
async fn payroll_update_cutoff(state: State<'_, AppState>, token: String, input: serde_json::Value) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_AUTH_REQUIRED".into()); }
    let id = input.get("payrollId").and_then(|v| v.as_str()).ok_or_else(|| "PAYROLL_NOT_FOUND".to_string())?;
    let parsed = cutoff_input(&input);
    let result = crate::services::cutoff_payroll::calculate(&parsed)?;
    let now = chrono::Utc::now().to_rfc3339();
...
async fn payroll_finalize_cutoff(state: State<'_, AppState>, token: String, payroll_id: String) -> Result<serde_json::Value, String> {
    ...
    if result.rows_affected() == 0 { return Err("PAYROLL_NOT_FOUND_OR_FINALIZED".into()); }
    enqueue_sync(&state, "PayrollCutoffs", &payroll_id, "UPSERT", &serde_json::json!({"payrollId":payroll_id,"status":"FINALIZED","finalizedAt":now})).await;
    Ok(serde_json::json!({"success":true,"payrollId":payroll_id,"status":"FINALIZED","finalizedAt":now}))
}
...
async fn payroll_export_csv(state: State<'_, AppState>, token: String) -> Result<String, String> {
    ...
    for row in rows { let name = row.get::<String,_>("employee_name").replace('"', "\"\""); let label = row.get::<String,_>("payroll_cutoff_label").replace('"', "\"\""); output.push_str(&format!("{},{},\"{}\",\"{}\",{},{},{:.2},{:.2},{}\n", row.get::<String,_>("payroll_id"),row.get::<String,_>("employee_id"),name,label,row.get::<String,_>("cutoff_start"),row.get::<String,_>("cutoff_end"),row.get::<i64,_>("gross_compensation_centavos") as f64/100.0,row.get::<i64,_>("net_pay_centavos") as f64/100.0,row.get::<String,_>("status"))); }
    Ok(output)
}
...
fn cutoff_input(value: &serde_json::Value) -> crate::services::cutoff_payroll::CutoffInput {
    ...
    crate::services::cutoff_payroll::CutoffInput { employee_id:value.get("employeeId").and_then(|v|v.as_str()).unwrap_or("").into(), employee_name:value.get("employeeName").and_then(|v|v.as_str()).unwrap_or("").into(), cutoff_start:value.get("cutoffStart").and_then(|v|v.as_str()).unwrap_or("").into(), cutoff_end:value.get("cutoffEnd").and_then(|v|v.as_str()).unwrap_or("").into(), daily_rate:n("dailyRate"), standard_working_days:n("standardWorkingDays"), actual_working_days:n("actualWorkingDays"), special_holiday_days:n("specialHolidayDays"), special_holiday_multiplier:n("specialHolidayMultiplier"), regular_holiday_days:n("regularHolidayDays"), regular_holiday_multiplier:n("regularHolidayMultiplier"), incentives_allowance:n("incentivesAllowance"), special_allowance:n("specialAllowance"), late_deduction:n("lateDeduction"), half_day_count:n("halfDayCount"), half_day_fraction:n("halfDayFraction"), absent_days:n("absentDays"), overtime_hours:n("overtimeHours"), overtime_rate:n("overtimeRate"), manual_adjustment:n("manualAdjustment"), adjustment_reason:value.get("adjustmentReason").and_then(|v|v.as_str()).map(str::to_owned), approved_working_day_overage:value.get("approvedWorkingDayOverage").and_then(|v|v.as_bool()).unwrap_or(false) }
}

#[tauri::command]
async fn setup_unlock(state: State<'_, AppState>, pin: String) -> Result<serde_json::Value, String> {
    let configured = state.lan.admin_pin.as_deref().ok_or_else(|| "ADMIN_DISABLED".to_string())?;
    if configured != pin { return Err("INVALID_ADMIN_PIN".into()); }
    let token = uuid::Uuid::new_v4().to_string();
    let expires = std::time::Instant::now() + std::time::Duration::from_secs(state.lan.admin_session_minutes * 60);
    *state.admin_session.lock().await = Some(AdminSession { token: token.clone(), expires_at: expires });
    Ok(serde_json::json!({"success":true,"token":token,"expiresAt":chrono::Utc::now() + chrono::Duration::minutes(state.lan.admin_session_minutes as i64)}))
}
...
#[tauri::command]
async fn setup_lookup_card(state: State<'_, AppState>, token: String, rfid_uid: String) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("SETUP_AUTH_REQUIRED".into()); }
...
#[tauri::command]
async fn setup_upsert_user(state: State<'_, AppState>, token: String, user: serde_json::Value) -> Result<serde_json::Value, String> {
    admin_upsert_user(state, token, user).await
}

#[tauri::command]
async fn admin_unlock(state: State<'_, AppState>, pin: String) -> Result<serde_json::Value, String> { setup_unlock(state, pin).await }

#[tauri::command]
async fn admin_get_session(state: State<'_, AppState>, token: String) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_SESSION_EXPIRED".into()); }
    Ok(serde_json::json!({"success":true}))
}

#[tauri::command]
async fn admin_lock(state: State<'_, AppState>) -> Result<serde_json::Value, String> { setup_lock(state).await }

#[tauri::command]
async fn admin_get_sync_status(state: State<'_, AppState>, token: String) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_AUTH_REQUIRED".into()); }
    let pending: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM sync_queue WHERE status IN ('PENDING','RETRY','PROCESSING')").fetch_one(&state.db).await.map_err(|e| e.to_string())?;
    let dead: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM sync_queue WHERE status='DEAD'").fetch_one(&state.db).await.unwrap_or(0);
    Ok(serde_json::json!({"success":true,"pending":pending,"deadLetter":dead}))
}

#[tauri::command]
async fn admin_retry_sync_item(state: State<'_, AppState>, token: String, id: i64) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_AUTH_REQUIRED".into()); }
    let now = chrono::Utc::now().to_rfc3339();
    sqlx::query("UPDATE sync_queue SET status='PENDING', attempts=0, last_error=NULL, next_attempt_at=?, updated_at=? WHERE id=?").bind(&now).bind(&now).bind(id).execute(&state.db).await.map_err(|e| e.to_string())?;
    Ok(serde_json::json!({"success":true,"id":id}))
}

#[tauri::command]
async fn admin_sync_now(state: State<'_, AppState>, token: String) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("ADMIN_AUTH_REQUIRED".into()); }
    let processed = crate::services::sheets_sync::run_once(&state, state.lan.sheets_sync_endpoint.as_deref()).await?;
...
async fn admin_authorized(state: &AppState, token: &str) -> bool {
    ...
    if let Some(value) = session.as_ref() { if value.token == token && value.expires_at > std::time::Instant::now() { return true; } }
    *session = None;
    false
}

async fn enqueue_sync(state: &AppState, table_name: &str, row_id: &str, operation: &str, payload: &serde_json::Value) {
    let now = chrono::Utc::now().to_rfc3339();
    let _ = sqlx::query("INSERT INTO sync_queue (table_name,row_id,operation,payload_json,attempts,next_attempt_at,created_at,updated_at) VALUES (?,?,?,?,0,?,?,?)")
        .bind(table_name).bind(row_id).bind(operation).bind(payload.to_string()).bind(&now).bind(&now).bind(&now).execute(&state.db).await;
}

#[tauri::command]
async fn scan_rfid(app: tauri::AppHandle, state: State<'_, AppState>, request: serde_json::Value) -> Result<serde_json::Value, String> {
    let request_id = uuid::Uuid::new_v4().to_string();
    let uid = request.get("rfidUid").and_then(|v| v.as_str()).unwrap_or("").trim().to_ascii_uppercase();
    let source = request.get("source").and_then(|v| v.as_str()).unwrap_or("RFID");
    ...
    let valid_uid = source == "MANUAL_TEST" || (uid.len() >= 4 && uid.len() <= 64 && uid.bytes().all(|byte| byte.is_ascii_hexdigit()));
    if uid.is_empty() || !valid_uid || !matches!(source, "RFID" | "MANUAL_TEST") {
        return Ok(serde_json::json!({"success":false,"requestId":request_id,"error":{"code":"INVALID_SCAN_INPUT","message":"rfidUid and source are required."}}));
    }
    let now = chrono::Utc::now();
    {
        let mut guard = state.scan_guard.lock().await;
        if guard.get(&uid).is_some_and(|last| last.elapsed().as_millis() < 500) {
            return Ok(serde_json::json!({"success":false,"requestId":request_id,"error":{"code":"DUPLICATE_SCAN","message":"This card was scanned too recently."}}));
        }
        guard.insert(uid.clone(), Instant::now());
    }
    {
        let cooldown = state.physical_cooldown.lock().await;
        if cooldown.get(&uid).is_some_and(|last| last.elapsed().as_secs() < 10) {
            return Ok(serde_json::json!({"success":false,"requestId":request_id,"error":{"code":"DUPLICATE_SCAN","message":"This card was scanned too recently.","retryAfterSeconds":10}}));
        }
    }
    ...
    if status != "ACTIVE" { return Ok(serde_json::json!({"success":false,"requestId":request_id,"error":{"code":"INACTIVE_USER","message":"This user is inactive."}})); }
    ...
    let (attendance_id, action, time_in, time_out, attendance_status) = match existing {
        None => {
            ...
            if result.is_err() { return Ok(serde_json::json!({"success":false,"requestId":request_id,"error":{"code":"INTERNAL_SERVER_ERROR","message":"Unable to save attendance."}})); }
        ...
        Some(row) => {
            ...
            if tout.is_some() { return Ok(serde_json::json!({"success":false,"requestId":request_id,"error":{"code":"ATTENDANCE_ALREADY_COMPLETED","message":"Attendance is already complete for today."}})); }
            if tin.is_none() { return Ok(serde_json::json!({"success":false,"requestId":request_id,"error":{"code":"ATTENDANCE_DATA_CONFLICT","message":"Attendance data is inconsistent."}})); }
            ...
            if result.map(|r| r.rows_affected()).unwrap_or(0) != 1 { return Ok(serde_json::json!({"success":false,"requestId":request_id,"error":{"code":"ATTENDANCE_DATA_CONFLICT","message":"Attendance changed before the scan was saved."}})); }
            (id, "TIME_OUT", tin, Some(timestamp.clone()), "COMPLETED")
        }
    };
    let seq = state.next_sequence();
    state.physical_cooldown.lock().await.insert(uid.clone(), Instant::now());
    if action == "TIME_OUT" {
        if let (Some(actual_in), Some(actual_out)) = (time_in.as_deref(), time_out.as_deref()) {
            ...
            if let Err(error) = ensure_payroll(&state, &attendance_id, &user_id, user.get::<String,_>("full_name"), &employee_type, daily_rate, &date, actual_in, actual_out).await {
                return Ok(serde_json::json!({"success":false,"requestId":request_id,"error":{"code":"PAYROLL_GENERATION_FAILED","message":error}}));
            }
        }
    }
    let _ = state.bus.sender.send(crate::lan_server::LanAttendanceEvent::AttendanceUpdated { event_id: format!("{}:{}", state.server_instance_id, seq), server_instance_id: state.server_instance_id.to_string(), sequence: seq, occurred_at: now, request_id: request_id.clone(), attendance_date: date.clone(), attendance_id: attendance_id.clone(), cause: action.to_string(), mutation: "upsert".into(), attendance: Some(crate::lan_server::LanAttendanceRow { attendance_id: attendance_id.clone(), attendance_date: date.clone(), user_id: user_id.clone(), full_name: user.get::<String,_>("full_name"), department: user.get::<Option<String>,_>("department"), time_in: time_in.clone(), time_out: time_out.clone(), status: attendance_status.to_string() }) });
    ...
    enqueue_sync(&state, "Attendance", payload.get("attendanceId").and_then(|v| v.as_str()).unwrap_or_default(), "UPSERT", &payload).await;
...
async fn ensure_payroll(state: &AppState, attendance_id: &str, user_id: &str, full_name: String, employee_type: &str, daily_rate: Option<i64>, date: &str, actual_in: &str, actual_out: &str) -> Result<(), String> {
    if sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM payroll WHERE attendance_id=?").bind(attendance_id).fetch_one(&state.db).await.map_err(|e| e.to_string())? > 0 { return Ok(()); }
    let (computed_in, computed_out, grace_used, late_hours, deduction, base_pay, daily_pay) = if employee_type == "EMPLOYEE" {
        let result = crate::services::employee_payroll::calculate(actual_in, actual_out, daily_rate.unwrap_or(0))?;
        (result.computed_time_in, result.computed_time_out, None, result.late_hours, result.late_deduction_centavos, result.base_pay_centavos, result.daily_pay_centavos)
    } else {
        let date_value = chrono::NaiveDate::parse_from_str(date, "%Y-%m-%d").map_err(|e| e.to_string())?;
        let week_start = date_value - chrono::Duration::days(date_value.weekday().num_days_from_monday() as i64);
        let grace_available = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM intern_grace WHERE user_id=? AND week_start=?").bind(user_id).bind(week_start.to_string()).fetch_one(&state.db).await.map_err(|e| e.to_string())? == 0;
        let result = crate::services::intern_payroll::calculate(date, actual_in, actual_out, grace_available)?;
        ...
        (result.computed_time_in, result.computed_time_out, Some(result.grace_used), result.late_hours, result.late_deduction_centavos, result.base_pay_centavos, result.daily_pay_centavos)
    };
    let now = chrono::Utc::now().to_rfc3339();
...
#[tauri::command]
async fn upload_photo(state: State<'_, AppState>, token: String, user_id: String, base64_data: String) -> Result<serde_json::Value, String> {
    if !admin_authorized(&state, &token).await { return Err("SETUP_AUTH_REQUIRED".into()); }
    if user_id.is_empty() || user_id.len() > 128 || !user_id.bytes().all(|byte| byte.is_ascii_alphanumeric() || matches!(byte, b'-' | b'_' | b'.')) { return Err("INVALID_USER_ID".into()); }
    use base64::Engine;
    let bytes = base64::engine::general_purpose::STANDARD.decode(base64_data.split_once(',').map(|(_, value)| value).unwrap_or(&base64_data)).map_err(|_| "INVALID_IMAGE".to_string())?;
    if bytes.len() > 500 * 1024 { return Err("IMAGE_TOO_LARGE".into()); }
    let image = image::load_from_memory(&bytes).map_err(|_| "INVALID_IMAGE_FORMAT".to_string())?;
    if image.width() > 512 || image.height() > 512 { return Err("IMAGE_DIMENSIONS_EXCEEDED".into()); }
    ...
    image.save_with_format(&path, image::ImageFormat::WebP).map_err(|e| e.to_string())?;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            let data_dir: PathBuf = app.path().app_local_data_dir().expect("application data directory");
            let config_dir = app.path().app_config_dir().expect("application config directory");
            ...
            let state = tauri::async_runtime::block_on(AppState::new(data_dir, lan)).expect("SQLite initialization");
            if state.lan.enabled {
                let server_state = state.clone();
                tauri::async_runtime::spawn(async move { let _ = lan_server::start(server_state).await; });
            }
        ...
        .invoke_handler(tauri::generate_handler![get_health, get_config, get_attendance, scan_rfid, upload_photo, admin_users, admin_list_users, admin_upsert_user, admin_create_user, admin_update_user, admin_delete_user, admin_attendance, admin_list_attendance, admin_update_attendance, admin_delete_attendance, payroll_calculate_cutoff, payroll_list_profiles, payroll_upsert_profile, payroll_list_cutoffs, payroll_create_cutoff, payroll_update_cutoff, payroll_finalize_cutoff, payroll_export_csv, setup_unlock, setup_lock, setup_lookup_card, setup_upsert_user, admin_unlock, admin_get_session, admin_lock, admin_get_sync_status, admin_retry_sync_item, admin_sync_now])
