...
async fn payroll_intern_report(
    ...
    let rows = sqlx::query(
        "SELECT u.user_id, u.full_name, \
         ...
         COALESCE(SUM(CASE WHEN p.is_half_day = 1 OR (p.actual_time_in IS NOT NULL AND p.actual_time_out IS NOT NULL AND ((strftime('%s', p.actual_time_out) - strftime('%s', p.actual_time_in)) <= 18000 OR substr(p.actual_time_out, 12, 5) < '17:00' OR substr(p.actual_time_in, 12, 5) >= '12:00')) THEN 1 ELSE 0 END), 0) AS half_day_count, \
...
async fn payroll_generate_cutoff(
    ...
    let rows = sqlx::query(
        "SELECT p.user_id, MAX(p.full_name) AS full_name, MAX(p.employee_type) AS employee_type, \
         ...
         SUM(CASE WHEN p.is_half_day = 1 OR (p.actual_time_in IS NOT NULL AND p.actual_time_out IS NOT NULL AND ((strftime('%s', p.actual_time_out) - strftime('%s', p.actual_time_in)) <= 18000 OR substr(p.actual_time_out, 12, 5) < '17:00' OR substr(p.actual_time_in, 12, 5) >= '12:00')) THEN 1 ELSE 0 END) AS half_day_count, \
